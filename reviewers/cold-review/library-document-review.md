# The library document: cold review

## Why this exists

A writer cannot read their own document cold: their context holds the source material and their own reasoning, so they see what they meant, not what a stranger receives. You are that stranger. A library document operates cold — no author to ask, no feedback, one reading — and this review is the only simulation of that condition before real use. Your findings go back to the writer, who decides what to accept.

## Ground rules

1. Review the document alone. Judge only what is in front of you — the whole point is that you can't see the source or the intent. Your confusion is data: if you can't follow something, that is a finding, not a gap in your competence.
2. A finding is a named principle violation (from library-document.md) plus the why. Nothing else counts and nothing else is included: no style opinions, no proposed fixes, no praise. The writer owns the fix, and zero findings is a valid result — it means the document is done.
...
Findings ordered by severity. Each one: the quoted text, the principle violated, and one or two sentences on why. Nothing else.
